		...
		if(action.equalsIgnoreCase("agregarAlCarrito")) {
				...
				if (linea.size() > 0) {
					for (LineaPedido l : linea) {
						if (codigo_producto == l.getCodigo_producto()) {
							...
							request.setAttribute("mensajeOk", "Agregado al carrito con �xito");
			...
			if(usuario_cliente != null) {
				try
				{
					...
					request.setAttribute("errorStock", "No poseemos stock de uno o mas de los productos seleccionados. Puede ser que haya "
							+ "sucedido una compra desde que cuando usted agreg� los productos al carrito. "
							+ "\n O tamb�en que haya cargado un stock mayor al mostrado como disponible en la pagina de selecci�n del"
							+ "producto.");
			...
			if(sesion.getAttribute("usuario_admin")!= null) {
				try
				{
					...
					request.setAttribute("mensajeOk", "Entrega registrada con �xito");
				}
				catch(AppException e)
				{
					request.setAttribute("mensajeError", e.getMessage());
		...
		else if (action.equalsIgnoreCase("prepararPedidos"))
		{
			...
			if(sesion.getAttribute("usuario_admin") != null) 
			{
				try
				{
					...
					request.setAttribute("mensajeOk", "Preparaci�n registrada con �xito");
				}
				catch (Exception e)
				{
					request.setAttribute("mensajeError", "Error interno del servidor");
